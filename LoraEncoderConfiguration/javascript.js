...
var langData = {
	...
	securedCom:["The report will be sent with an acknowledgment request","Le rapport sera envoyé avec demande d'acquittement"],
			...
			if (err !== null) {
				...
				availableProducts.products.forEach(function(product){
					...
					if(typeof(product["apps"]) !== 'undefined') {
						displayed = product.apps.includes("LoraEncoderConfiguration");
					}
					if (displayed) {
		...
		currentCommandParameters.forEach(function(element) {
			...
			if ((element.editable) ||(selectable)) { //Si le paramètre est modifiable ou sélectionable on le montre
				if(element.type == "data" || element.type == "array") {
					...
					addParameterRow(body,i,parameterIndex,element,undefined,clusterData.TICAttributeInstances); //Appel de notre fonction d'ajout de ligne
					parameterIndex++; //On incrémente le numéro d'index du paramètre
					i++; //On incrémente "i"
				}
			}
